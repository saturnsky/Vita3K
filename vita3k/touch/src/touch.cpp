...
static SceTouchData recover_touch_events(const EmuEnvState &emuenv) {
    ...
    for (uint8_t i = 0; i < finger_count; i++) {
        ...
        float x = (finger_buffer[i].x * emuenv.drawable_size.x - emuenv.logical_viewport_pos.x) / emuenv.logical_viewport_size.x;
        float y = (finger_buffer[i].y * emuenv.drawable_size.y - emuenv.logical_viewport_pos.y) / emuenv.logical_viewport_size.y;
        ...
        for (int port = 0; port < 2; port++) {
            ...
            if ((buttons & mask) && emuenv.renderer_focused) {
                ...
                const SceFVector2 touch_pos_viewport = {
                    (touch_pos_drawable.x - emuenv.logical_viewport_pos.x) / emuenv.logical_viewport_size.x,
                    (touch_pos_drawable.y - emuenv.logical_viewport_pos.y) / emuenv.logical_viewport_size.y
                };
