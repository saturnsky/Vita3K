...
void convert_yuv_to_rgb(const uint8_t *yuv, uint8_t *rgba, uint32_t width, uint32_t height) {
    ...
    const uint8_t *slices[] = {
        ...
        &yuv[static_cast<uint32_t>(width * height * 2)], // V Slice
    };
...
bool MjpegDecoderState::receive(uint8_t *data, DecoderSize *size) {
    ...
    if (data) {
        ...
        case AV_PIX_FMT_YUVJ420P: {
            std::vector<uint8_t> temporary(frame->width * frame->height * 6 / 4);
            uint8_t *channels[] = {
                &temporary.data()[0], // y
                &temporary.data()[frame->width * frame->height], // u
                &temporary.data()[frame->width * frame->height * 5 / 4], // v
            };
            ...
            convert_yuv420_to_yuv444(temporary.data(), data, frame->width, frame->height);
            break;
        }
        default:
