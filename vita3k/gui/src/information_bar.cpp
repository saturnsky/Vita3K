...
static void draw_notice_info(GuiState &gui, EmuEnvState &emuenv) {
    ...
    const ImVec2 NOTICE_SIZE = notice_info_count_new ? ImVec2(104.0f * RES_SCALE.x, 95.0f * RES_SCALE.y) : ImVec2(90.0f * RES_SCALE.x, 82.0f * RES_SCALE.y);
    ...
    const ImVec2 WINDOW_SIZE = notice_info_state ? VIEWPORT_SIZE : (notice_info_count_new ? ImVec2(84.f * RES_SCALE.x, 76.f * RES_SCALE.y) : ImVec2(72.f * RES_SCALE.x, 62.f * RES_SCALE.y));
    ...
    ImGui::PushStyleVar(ImGuiStyleVar_WindowRounding, 50.f * RES_SCALE.x);
    ...
    if (notice_info_count_new) {
        if (gui.theme_information_bar_notice.contains(NoticeIcon::NEW))
            ...
            draw_list->AddCircleFilled(ImVec2(VIEWPORT_WIDTH_POS_MAX - (24.f * RES_SCALE.x), VIEWPORT_POS.y + (16.f * RES_SCALE.y)), 60.f * RES_SCALE.x, IM_COL32(11.f, 90.f, 252.f, 255.f));
        const auto FONT_SCALE = 40.f * RES_SCALE.x;
        ...
        draw_list->AddText(gui.vita_font, FONT_SCALE, ImVec2(VIEWPORT_WIDTH_POS_MAX - (NOTICE_SIZE.x / 2.f) - (NOTICE_COUNT_SIZE / 2.f) + (12.f * RES_SCALE.x), VIEWPORT_POS.y + (15.f * RES_SCALE.y)), NOTICE_COLOR, std::to_string(notice_info_count_new).c_str());
    } else {
        if (gui.theme_information_bar_notice.contains(NoticeIcon::NO))
            ...
            draw_list->AddCircleFilled(ImVec2(VIEWPORT_WIDTH_POS_MAX - (28.f * RES_SCALE.x), VIEWPORT_POS.y + (18.f * RES_SCALE.y)), 44.f * RES_SCALE.x, IM_COL32(62.f, 98.f, 160.f, 255.f));
    }
    ...
    if (notice_info_state) {
        ...
        ImGui::PushStyleVar(ImGuiStyleVar_ChildBorderSize, notice_info.empty() ? 0.f : 8.0f * RES_SCALE.x);
        ...
        if (notice_info.empty()) {
            ...
            ImGui::SetColumnWidth(0, 85.f * RES_SCALE.x);
            const auto ICON_SIZE = ImVec2(64.f * RES_SCALE.x, 64.f * RES_SCALE.y);
            const auto SELECT_SIZE = ImVec2(POPUP_SIZE.x, 80.f * RES_SCALE.y);
            ...
            for (const auto &notice : notice_info) {
                ...
                ImGui::SetCursorPosY(ImGui::GetCursorPosY() + (14.f * RES_SCALE.y));
                ...
                ImGui::SetCursorPos(ImVec2(POPUP_SIZE.x - (34.f * RES_SCALE.x) - notice_time_size.x, ImGui::GetCursorPosY() - (8.f * RES_SCALE.y)));
        ...
        if (!notice_info.empty()) {
            const auto DELETE_POPUP_SIZE = ImVec2(756.0f * RES_SCALE.x, 436.0f * RES_SCALE.y);
            ...
            ImGui::SetCursorPos(ImVec2(VIEWPORT_SIZE.x - (70.f * RES_SCALE.x), VIEWPORT_SIZE.y - (52.f * RES_SCALE.y)));
            if (ImGui::Button("...", ImVec2(64.f * RES_SCALE.x, 40.f * RES_SCALE.y)) || ImGui::IsKeyPressed(static_cast<ImGuiKey>(emuenv.cfg.keyboard_button_triangle)))
                ImGui::OpenPopup("...");
            if (ImGui::BeginPopup("...", ImGuiWindowFlags_NoMove)) {
                ...
                if (ImGui::BeginPopupModal("Delete All", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoSavedSettings)) {
                    ...
                    ImGui::SetCursorPosY((DELETE_POPUP_SIZE.y / 2.f) - (46.f * RES_SCALE.y));
                    TextColoredCentered(GUI_COLOR_TEXT, lang["notif_deleted"].c_str());
                    ImGui::SetCursorPos(ImVec2((DELETE_POPUP_SIZE.x / 2) - (BUTTON_SIZE.x + (20.f * RES_SCALE.x)), DELETE_POPUP_SIZE.y - BUTTON_SIZE.y - (24.0f * RES_SCALE.y)));
...
void draw_information_bar(GuiState &gui, EmuEnvState &emuenv) {
    ...
    const ImVec2 INFO_BAR_SIZE(VIEWPORT_SIZE.x, 32.f * RES_SCALE.y);
    ...
    const auto is_notif_pos = !gui.vita_area.start_screen && (gui.vita_area.live_area_screen || gui.vita_area.home_screen) ? 78.f * RES_SCALE.x : 0.f;
    ...
    if (gui.vita_area.home_screen || gui.vita_area.live_area_screen) {
        const auto HOME_ICON_POS_CENTER = VIEWPORT_POS.x + (INFO_BAR_SIZE.x / 2.f) - (32.f * ((float(gui.live_area_current_open_apps_list.size())) / 2.f)) * RES_SCALE.x;
        const auto APP_IS_OPEN = gui.live_area_app_current_open >= 0;

        // Draw Home Icon
        const std::vector<ImVec2> HOME_UP_POS = {
            ImVec2(HOME_ICON_POS_CENTER - (13.f * RES_SCALE.x), VIEWPORT_POS.y + (16.f * RES_SCALE.y)),
            ImVec2(HOME_ICON_POS_CENTER, VIEWPORT_POS.y + (6.f * RES_SCALE.y)),
            ImVec2(HOME_ICON_POS_CENTER + (13.f * RES_SCALE.x), VIEWPORT_POS.y + (16.f * RES_SCALE.y))
        };
        const auto HOME_DOWN_POS_MINI = ImVec2(HOME_ICON_POS_CENTER - (8.f * RES_SCALE.x), VIEWPORT_POS.y + (16.f * RES_SCALE.y));
        const auto HOME_DOWN_POS_MAX = ImVec2(HOME_ICON_POS_CENTER + (8.f * RES_SCALE.x), HOME_DOWN_POS_MINI.y + (10.f * RES_SCALE.y));
        ...
        draw_list->AddRectFilled(ImVec2(HOME_ICON_POS_CENTER - (3.f * RES_SCALE.x), VIEWPORT_POS.y + (18.5f * RES_SCALE.y)), ImVec2(HOME_ICON_POS_CENTER + (3.f * RES_SCALE.x), VIEWPORT_POS.y + (26.f * RES_SCALE.y)), bar_color);

        // Draw App Icon
        const float decal_app_icon_pos = 34.f * ((float(gui.live_area_current_open_apps_list.size()) - 2) / 2.f);
        const auto ICON_SIZE_SCALE = 28.f * RES_SCALE.x;

        for (auto a = 0; a < gui.live_area_current_open_apps_list.size(); a++) {
            const ImVec2 ICON_POS_MIN(VIEWPORT_POS.x + (INFO_BAR_SIZE.x / 2.f) - (14.f * RES_SCALE.x) - (decal_app_icon_pos * RES_SCALE.x) + (a * (34 * RES_SCALE.x)), VIEWPORT_POS.y + (2.f * RES_SCALE.y));
    ...
    const ImVec2 CLOCK_POS(VIEWPORT_POS.x + INFO_BAR_SIZE.x - (64.f * RES_SCALE.x) - CLOCK_SIZE_SCALE.x - DAY_MOMENT_SIZE_SCALE.x - is_notif_pos, VIEWPORT_POS.y + (INFO_BAR_SIZE.y / 2.f) - (CLOCK_SIZE_SCALE.y / 2.f));
    const auto DAY_MOMENT_POS = ImVec2(CLOCK_POS.x + CLOCK_SIZE_SCALE.x + (6.f * RES_SCALE.x), CLOCK_POS.y + (CLOCK_SIZE_SCALE.y - DAY_MOMENT_SIZE_SCALE.y));
    ...
    const auto FULL_BATTERY_SIZE = 38.f * RES_SCALE.x;
    const auto BATTERY_HEIGHT_MIN_POS = VIEWPORT_POS.y + (5.f * RES_SCALE.y);
    const ImVec2 BATTERY_MAX_POS(VIEWPORT_POS.x + INFO_BAR_SIZE.x - (12.f * RES_SCALE.x) - is_notif_pos, BATTERY_HEIGHT_MIN_POS + (22 * RES_SCALE.y));
    const ImVec2 BATTERY_BASE_MIN_POS(BATTERY_MAX_POS.x - FULL_BATTERY_SIZE - (4.f * RES_SCALE.x), VIEWPORT_POS.y + (12.f * RES_SCALE.y));
    const ImVec2 BATTERY_BASE_MAX_POS(BATTERY_BASE_MIN_POS.x + (4.f * RES_SCALE.x), BATTERY_BASE_MIN_POS.y + (8 * RES_SCALE.y));
    ...
    draw_list->AddRectFilled(ImVec2(BATTERY_MAX_POS.x - FULL_BATTERY_SIZE, BATTERY_HEIGHT_MIN_POS), BATTERY_MAX_POS, BATTERY_BG_COLOR, 2.f * RES_SCALE.x, ImDrawFlags_RoundCornersAll);
    ...
    draw_list->AddRectFilled(ImVec2(BATTERY_MAX_POS.x - battery_size, BATTERY_HEIGHT_MIN_POS), BATTERY_MAX_POS, BATTERY_COLOR, 2.f * RES_SCALE.x, ImDrawFlags_RoundCornersAll);
